 ...
 *
 * 왜 뺐나 — 아래 원본 주석이 스스로 자백하고 있다:
 * "01 = CompanyChapters 02장과 같은 절차 / 02 = 01장과 같은 얘기 / 03 = 03장 문장을 그대로 쓴다".
 * 즉 원칙 3줄과 공정 3장은 **같은 말**이었고 독자는 같은 얘기를 두 번 읽었다.
 * 게다가 그 중복 구간에 화면 높이 2.2배의 스크롤을 배정했다 —
 * 정보를 찾으러 온 담당 공무원에게 **새 정보가 0인 2.2화면**은 이탈 구간이다.
 * CEO 반려 3회 중 2차 개정에서 넣은 부품이고, 3차 반려의 원인 중 하나로 판단해 뺐다.
 *
 * 되돌릴 수 있게 파일만 남긴다(`LedBoard.tsx` 선례). 참조가 0이라 번들에는 들어가지 않는다.
 * 🔴 다시 배선하려면 **CompanyChapters 와의 중복을 먼저 해소**하라. 그러지 않으면 같은 반려가 온다.
 */
/**
