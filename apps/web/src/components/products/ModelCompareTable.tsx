 ...
 * 방진·방수 / 보는 거리. 밝기·소비전력처럼 모델을 정한 뒤에 보는 값은 각 시리즈 상세 규격표에 있다.
 *
    ...
    <section className="wk-sec bg-white" aria-labelledby="cmp-h">
      <div className="wk-wrap">
              ...
              <tbody>
                {rows.map((m, i) => (
                  <tr key={m.slug} className={i % 2 === 1 ? 'bg-wk-bgFaint' : 'bg-white'}>
                    ...
                    <td className={`${TD} wk-metric border-t border-wk-line`}>{m.pitches.length}종</td>
                    ...
                    <td className={`${TD} wk-metric border-t border-wk-line`}>{viewingHint(m)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <p className="wk-cap mt-4">
          ...
          둡니다. 보는 거리는 화소 간격 1mm 당 약 1m 로 잡은 어림값이며 확정 사양이 아닙니다.
        </p>
      </div>
    </section>
  )
