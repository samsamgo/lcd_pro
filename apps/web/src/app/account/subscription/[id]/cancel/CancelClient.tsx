...
export function CancelClient({ subscriptionId, customerId, status, createdAt, nextBillingAt }: Props) {
  ...
  return (
    <div className="mx-auto max-w-md p-6">
      ...
      {alreadyCanceled ? (
        <div className="mt-6 rounded-xl border border-zinc-300 bg-white/40 p-5">
          <p className="text-sm text-zinc-600">이 구독은 이미 해지된 상태입니다.</p>
        </div>
      ) : (
        <>
          <div className="mt-6 space-y-3 rounded-xl border border-yellow-500/30 bg-yellow-500/5 p-5 text-sm">
            ...
            <ul className="space-y-1 text-zinc-700">
              <li>· 해지 후 <span className="text-zinc-900">다음 결제 사이클부터 청구가 중단</span>됩니다.</li>
              {within7Days ? (
              ...
              {nextBillingAt && (
                <li>· 다음 결제 예정일: <span className="text-zinc-900">{new Date(nextBillingAt).toLocaleDateString('ko-KR')}</span></li>
              )}
          ...
          <div className="mt-6 space-y-4">
            <div>
              <label className="mb-2 block text-sm font-medium text-zinc-700">해지 사유</label>
              <div className="space-y-2">
                {REASONS.map((r) => (
                  <label key={r} className="flex cursor-pointer items-center gap-2 text-sm text-zinc-700">
                    <input
                      type="radio"
                      name="reason"
                      value={r}
            ...
            <div>
              <label className="mb-2 block text-sm font-medium text-zinc-700">추가 의견 (선택)</label>
              <textarea
                value={detail}
                onChange={(e) => setDetail(e.target.value)}
                rows={3}
                className="w-full rounded-lg border border-zinc-300 bg-white/40 p-3 text-sm text-zinc-900 placeholder:text-zinc-500"
            ...
            <div className="flex gap-3">
              <button
                onClick={() => router.back()}
                disabled={loading}
                className="flex-1 rounded-lg border border-zinc-300 px-4 py-3 text-sm text-zinc-700 hover:bg-zinc-100 disabled:opacity-50"
              >
                돌아가기
              </button>
              <button
