import type { Sku } from './pricing'
import { FAMILIES } from './standardBlock'
import { IMAGES } from '@/lib/imageAssets'

export interface ProductInfo {
  sku: Sku
  name: string
  tag: string
  env: 'indoor' | 'outdoor'
  pitch: string
  brightness: string
  /** 방수·방진 등급. 실내 제품은 등급 대상이 아니다 */
  ingress: string
  img: string
  imgAlt: string
  generated?: boolean
  highlights: string[]
  /** 권장 시청 거리 */
  viewingDistance: string
  /** 상세 모달용 한 줄 설명 */
  summary: string
  /** 추천 업종/공간 */
  bestFor: string[]
}

/** 제품 라인업 단일 소스 — ProductSection, 제품 상세 모달, Product JSON-LD 공용 */
export const PRODUCTS: ProductInfo[] = [
  {
    sku: 'IN-S',
    name: '창구·접수대 안내판',
    tag: '3m 안팎, 서서 보는 자리',
    env: 'indoor',
    pitch: 'P3',
    brightness: '800 nit',
    ingress: '실내 전용 · 방수 등급 대상 아님',
    img: IMAGES.productCards['IN-S'],
    imgAlt: '한국 공공 민원실 대기 공간 벽면의 대기 순번 안내 화면',
    generated: true,
    highlights: ['창구명·대기번호를 화면에서 바로 수정', '작은 글자가 뭉치지 않는 화소 간격', '실내 조명에 맞춘 밝기'],
    viewingDistance: '약 3m 이상',
    summary: '창구나 접수대 바로 앞에서 보는 안내판입니다. 코앞에서 보니까 화소가 촘촘해야 하고, 대기번호나 창구 안내는 담당자분이 화면에서 바로 고치시면 됩니다.',
    bestFor: ['창구·접수대', '민원실', '병원 원무과'],
  },
  {
    sku: 'IN-M',
    name: '로비·대기실 안내판',
    tag: '몇 발짝 떨어져 보는 자리',
    env: 'indoor',
    pitch: 'P3',
    brightness: '1000 nit',
    ingress: '실내 전용 · 방수 등급 대상 아님',
    img: IMAGES.productCards['IN-M'],
    imgAlt: '한국 공공 도서관 로비 벽면의 세로형 이용·운영시간·시설 안내 화면',
    generated: true,
    highlights: ['공지와 일정을 한 화면에', '안내 영상 재생', '시간대별로 다른 화면 예약'],
    viewingDistance: '약 4m 이상',
    summary: '로비나 대기실처럼 몇 발짝 떨어져서 보는 자리입니다. 그만큼 화면을 키울 수 있어서 시설 안내와 일정, 홍보 영상까지 한 화면에 담습니다.',
    bestFor: ['기관 로비', '학교 복도', '병원 대기실'],
  },
  {
    sku: 'OUT-S',
    name: '출입구·주차장 안내판',
    tag: '밖에 거는 작은 화면',
    env: 'outdoor',
    pitch: 'P4',
    brightness: '5000 nit',
    ingress: 'IP65 (전면·후면)',
    img: IMAGES.productCards['OUT-S'],
    imgAlt: '비 오는 저녁 한국 공공시설 진입로의 기둥형 호우 주의 LED 안내 화면',
    generated: true,
    highlights: ['햇빛 아래에서도 읽히는 밝기', '전면·후면 IP65', '안내 내용 원격 변경'],
    viewingDistance: '약 5m 이상',
    summary: '출입구나 주차장처럼 건물 밖에 놓는 안내판입니다. 실내용 밝기로는 낮에 안 보입니다. 햇빛을 이기는 밝기에 방수까지 함께 잡습니다.',
    bestFor: ['건물 출입구', '주차장 진입로', '학교 정문'],
  },
  {
    sku: 'OUT-M',
    name: '도로변·게시대 화면',
    tag: '차에서 읽는 크기',
    env: 'outdoor',
    pitch: 'P5',
    brightness: '6000 nit',
    ingress: 'IP65 (전면·후면)',
    img: IMAGES.productCards['OUT-M'],
    imgAlt: '비 오는 저녁 한국 도로 위 전자현수막에 표시된 호우 안전 안내',
    generated: true,
    highlights: ['낮에도 읽히는 밝기', '약 30m 거리에서 내용 확인', '전면·후면 IP65'],
    viewingDistance: '약 30m 이상',
    summary: '도로변이나 현수막 게시대 자리처럼 멀리서 보는 화면입니다. 차가 지나가면서 보는지 사람이 서서 보는지, 얼마나 높이 다는지에 따라 글자 크기가 달라집니다.',
    bestFor: ['전자현수막 게시대', '도로변 안내', '단지 입구'],
  },
  {
    sku: 'OUT-L',
    name: '건물 외벽 대형 화면',
    tag: '건물 외벽만 한 크기',
    env: 'outdoor',
    pitch: 'P6',
    brightness: '7000 nit',
    ingress: 'IP65 (전면·후면)',
    img: IMAGES.productCards['OUT-L'],
    imgAlt: '한국 공공 체육시설 외벽에 설치된 대형 옥외 LED 안내 화면',
    generated: true,
    highlights: ['넓은 외벽에 맞춘 취부 설계', '약 50m 밖에서 내용 확인', '나중에 정비하러 어디로 올라갈지를 설치 전에 함께 정합니다'],
    viewingDistance: '약 50m 이상',
    summary: '건물 외벽이나 넓은 부지에 거는 대형 화면입니다. 크기가 커지면 벽이 무게를 견디는지부터 봐야 하고, 밤에 눈부시지 않게 밝기도 잡아야 합니다. 나중에 정비하러 어디로 올라갈지도 미리 정해 둡니다.',
    bestFor: ['건물 외벽', '체육시설', '옥외 광장'],
  },
  {
    sku: 'P2.5',
    name: '근거리 정밀 안내판',
    tag: '표와 작은 글자가 많을 때',
    env: 'indoor',
    pitch: 'P2.5',
    brightness: '1200 nit',
    ingress: '실내 전용 · 방수 등급 대상 아님',
    img: IMAGES.productCards['P2.5'],
    imgAlt: '한국 공공 민원실 창구 상단에서 대기번호와 작은 창구명을 표시하는 정밀 안내 화면',
    generated: true,
    highlights: ['2m 앞에서도 또렷한 글자', '표와 도표가 뭉개지지 않음', '회의 자료·안내 영상 재생'],
    viewingDistance: '약 2m 이상',
    summary: '회의실이나 창구처럼 두어 걸음 앞에서 보는 자리에 맞춘 정밀형입니다. 표나 작은 글자가 많은 자료를 띄울 때 뭉치지 않습니다.',
    bestFor: ['회의실', '민원실 창구', '종합상황실'],
  },
]

/**
 * 화소 간격별 최대 소비전력(W/m²).
 *
 * 출처는 견적엔진의 제품군 정의(`standardBlock.ts` FAMILIES) 하나뿐이다.
 * 여기서 새로 만든 숫자는 없다. 엔진에 등록되지 않은 피치(P4·P6)는
 * 값을 지어내지 않고 비워 둔다 — 공급사 사양서 회수 후 채운다.
 *
 * ⚠ 엔진의 모든 제품군이 `candidate: true` 다. 즉 이 값은 확정 사양이 아니라
 *   전기 용량 산정을 위한 설계 상한(보수 추정)이다. 화면에 반드시 그렇게 표기한다.
 */
export const MAX_W_PER_M2_BY_PITCH: Record<string, number> = Object.fromEntries(
  Object.values(FAMILIES).map((f) => [f.pitch, f.max_w_per_m2]),
)

/** 그 피치의 소비전력 값이 확정 사양인가 (전부 추정이면 false) */
export const POWER_IS_CANDIDATE = Object.values(FAMILIES).some((f) => f.candidate)
