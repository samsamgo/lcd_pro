  ...
  theme: {
    extend: {
      ...
      colors: {
         ...
         */
        wk: {
          /* 🔴 2026-09-08 CEO 지시 — "파란색 계열보다 우강테크 로고 색을 따라."
             토큰 **이름은 그대로 두고 값만** 로고 주황 계열로 바꿨다. 사이트 전역에서
             wk-blue / wk-cta 를 참조하고 있어, 이름을 바꾸면 수백 군데를 손대야 하고
             그 과정에서 빠뜨린 곳이 파란색으로 남는다. 값만 바꾸면 한 번에 전부 따라온다.
             (이름이 색을 뜻하지 않게 된 건 감수한다 — 다음 정리 때 accent 로 개명할 것) */
          blue: '#DE671D',       // 로고 주황. 아이콘·강조선·큰 활자용
          blueHover: '#C75A16',
          blueActive: '#B14E11',
          blueWeak: '#FDF0E6',   // 2차 버튼 색면 (주황 계열 연한 면)
          ink: '#191F28',        // 1차 텍스트
          ...
          /** 흰 글자 CTA 전용 — 로고 주황 #DE671D 는 흰 글자와 3.46:1 로 AA 미달이라
              버튼 배경에는 한 단계 어두운 값을 쓴다 (계산해서 고른 값이다) */
          cta: '#B14E11',        // 흰 글자 5.29:1 — AA 통과
          ctaHover: '#9C440F',
          ctaActive: '#87390D',
          ink2: '#4E5968',       // 강조 본문
          ...
          warn: '#FE9800',
          bad: '#F04452',
          /** 로고 실측값 (자료/우강테그 로코.svg). 위 blue/cta 가 이 색에서 파생됐다 */
          logo: '#de671d',
          logoInk: '#03111f',
          logoGrey: '#6c7073',
        },
      ...
      boxShadow: {
        ...
        'wk-glow': '0 0 0 1px rgba(222,103,29,.18), 0 24px 80px -32px rgba(222,103,29,.55)',
      },
      keyframes: {
        'wk-marquee': {
